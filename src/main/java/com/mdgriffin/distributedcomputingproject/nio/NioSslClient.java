...
public class NioSslClient extends NioSslPeer {
    ...
    public NioSslClient(String protocol, String remoteAddress, int port) throws Exception  {
        ...
        context.init(createKeyManagers("./src/main/resources/ssl/client.jks", "storepass", "keypass"), createTrustManagers("./src/main/resources/ssl/trustedCerts.jks", "storepass"), new SecureRandom());
            ...
            if (bytesRead > 0) {
                ...
                while (peerNetData.hasRemaining()) {
                    ...
                    switch (result.getStatus()) {
                        case OK:
                            peerAppData.flip();
                            log.debug("SocketServer response: " + new String(peerAppData.array()));
                            exitReadLoop = true;
                            break;
                        case BUFFER_OVERFLOW:
